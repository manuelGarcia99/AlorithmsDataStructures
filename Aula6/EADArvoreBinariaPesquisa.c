...
PNodoAB removerNodoABP (PNodoAB T){
  ...
  if (alturaAB(T->Esquerda) < alturaAB(T->Direita))
    // 1º caso: remover o nodo sucessor (nodo mais à esquerda da subárvore direita) e copiar a sua informação
    T->Direita = substituirNodoDireita(T->Direita, &X);
  else
    // 2º caso: remover o nodo antecessor (nodo mais à direita da subárvore esquerda) e copiar a sua informação
    T->Esquerda = substituirNodoEsquerda(T->Esquerda, &X);
  T->Elemento = X;
