...
void main(){
    ...
    while(p != NULL){
        ...
        printf("%d\t %.2f \t\t %.2f\t\t %s\n",p->Elemento.numAluno,p->Elemento.notaFreq,p->Elemento.notaPratica,
        (p->Elemento.aprovado) ? "Aprovado" : "Reprovado");

        p= p->Prox;
    }
    float nota = L->Elemento.notaFreq;
    int numeroDeBonsAlunos = numDeElementosGrandeNF(L, nota);
    printf("\nNumero de alunos com nota superior ou igual ao primeiro elemento (%.2f): %d \n",nota,numeroDeBonsAlunos);

}

PNodo criarNodo2(INFO X){
  PNodo P;
...
PNodo inserirListaInicio2(INFO X , PNodo L){
    PNodo P;
    P = criarNodo(X);
    if (P == NULL)
        return L;
